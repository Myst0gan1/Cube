...
{
    // Clear the storage directory path used by the wallet
    string walletStoragePath = Path.Combine(Application.persistentDataPath, "Thirdweb", "EcosystemWallet");
    if (Directory.Exists(walletStoragePath))
    {
        Directory.Delete(walletStoragePath, true);
        Debug.Log("Wallet storage cleared successfully.");
    }
    else
    {
        Debug.LogWarning("No wallet storage directory found to clear.");
    }

    // Clear the active wallet reference
    ActiveWallet = null;
    Debug.Log("Active wallet reset.");
}
